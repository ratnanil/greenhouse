...
## ------------------------------------------------------------------------
ggplot(logger_sf) + 
  geom_sf(data = tische_sf,inherit.aes = F) +
  geom_sf() +
  geom_sf_text(aes(label = id1),nudge_x = 0.10,nudge_y = 0.15) +
  coord_sf()


library(plotly)

plot_ly(logger_sf2, x = ~x, y = ~y, z = ~z) %>%
  add_markers(text = ~id2, hoverinfo = "text")
...
# 
# 
minval <- min(map_dbl(interpolated$raster,~min(.x$var1.pred)))
maxval <- max(map_dbl(interpolated$raster,~max(.x$var1.pred)))
