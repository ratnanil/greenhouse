...
max(map_dbl(interpolated$raster,function(x){max(x$var1.pred)}))

theme_set(theme_light())
interpolated %>%
  head(10) %>%
  group_nest(Datum,.key = "dat") %>%
  group_by(Datum2 = as.Date(Datum)) %>%
  mutate(
  ...
  pmap(function(Datum,dat,maxval,minval,Datum2){
    path <- "Plots/"
      filename_i <- Datum %>%
        str_replace_all("-| |:","_") %>%
        paste0(.,".png")
      ...
      data_i <- dat %>%
        dplyr::select(tisch,level,data) %>%
        pmap_dfr(function(tisch,level,data){mutate(data,tisch = tisch, level = level)})

      raster_i <- dat %>%
        dplyr::select(tisch,level,raster) %>%
        pmap_dfr(function(tisch,level,raster){mutate(raster,tisch = tisch, level = level)})
      
      ggplot() +
        geom_raster(data = raster_i,aes(coords.x1,coords.x2,fill = var1.pred)) +
        coord_equal(1) +
        geom_sf(data = tische_sf,aes(colour = art), fill = NA,lwd = 2) +
        geom_point(data = data_i, aes(x,y),inherit.aes = F) +
        geom_text(data = data_i, aes(x,y,label = Temperatur),inherit.aes = F,nudge_x = 0.15,nudge_y = 0.15) +
        scale_fill_viridis_c(name = "Temperatur",limits = c(minval,maxval)) +
        facet_grid(level~tisch) +
        labs(title = Datum,colour = "Art")+
        theme(axis.title = element_blank(),
              axis.text = element_blank())

      width = 30
      ggsave(filepath,height = width/1.5,width = width,units = "cm")
  })
